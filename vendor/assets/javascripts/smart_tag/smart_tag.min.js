...
f=b.length-1;0<=f;f--)c(d,g(b[f]))};a.all_options=a.getOptions(k.children("option"));a.selected_options=a.getOptions(k.children("option:selected"));a.ul=g("<ul></ul>");k.before(a.ul);a.select.change(function(){a.changeTags()});a.start=function(){a.createTags();a.is_start=!0};return a}function n(k){var a=m(k);a.is_add=a.select.attr("is_add")?a.select.attr("is_add"):!0;a.ajaxAddTag=function(d){var c=a.select.attr("controller");if(c){a.select.attr("action")&&a.select.attr("action");var b=a.select.attr("param")?
a.select.attr("param"):"title";a.select.attr("controllers")&&a.select.attr("controllers");var f=a.select.attr("url"),e={};e[c+"["+b+"]"]=d;g.ajax({url:f,dataType:"json",type:"POST",data:e,success:function(a){a?k.prepend('<option selected="selected" value="'+a.id+'">'+d+"</option>"):console.log("josn is null")}})}else console.log("select's controller param is null")};a.ul.tagit({allowSpaces:!0,availableTags:a.all_options,afterTagAdded:function(d,c){var b=c.tagLabel;all_options=a.getOptions(a.select.children("option"));
