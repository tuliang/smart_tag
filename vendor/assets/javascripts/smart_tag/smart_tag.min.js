...
g=b.length-1;0<=g;g--)c(d,f(b[g]))};a.all_options=a.getOptions(k.children("option"));a.selected_options=a.getOptions(k.children("option:selected"));a.ul=f("<ul></ul>");k.before(a.ul);a.select.change(function(){a.changeTags()});a.start=function(){a.createTags();a.is_start=!0};return a}function n(k){var a=m(k);a.is_add=a.select.attr("is_add")?a.select.attr("is_add"):!0;a.ajaxAddTag=function(d){var c=a.select.attr("controller");if(c){var b=a.select.attr("action")?a.select.attr("action"):"create",g=a.select.attr("param")?
a.select.attr("param"):"title",b="/"+(a.select.attr("controllers")?a.select.attr("controllers"):c+"s")+"/"+b,e={};e[c+"["+g+"]"]=d;f.ajax({url:b,dataType:"json",type:"POST",data:e,success:function(a){a?k.prepend('<option selected="selected" value="'+a.id+'">'+d+"</option>"):console.log("josn is null")}})}else console.log("select's controller param is null")};a.ul.tagit({allowSpaces:!0,availableTags:a.all_options,afterTagAdded:function(d,c){var b=c.tagLabel;all_options=a.getOptions(a.select.children("option"));
...
a.getOptions(a.select.children("option"));if(a.inArray(all_options,b)){a.changeSelected(a.ul.tagit("assignedTags"),a.addSelected);var g=f(e.children("option:selected")[0]).val(),h=f("#careers_"+g).attr("nil_params");""==h?(a.ul.parents(".control-group").removeClass("error"),a.ul.parents(".control-group").addClass("success"),a.ul.parents(".control-group").find(".help-inline").text("Success!")):a.ul.parents(".control-group").find(".help-inline").html(h+' <a href="/careers/'+g+'/edit">Edit '+b+"</a>")}else a.ul.tagit("removeAll"),
a.ul.parents(".control-group").removeClass("success"),a.ul.parents(".control-group").addClass("error"),a.ul.parents(".control-group").find(".help-inline").text("You should choice one career.");1==a.ul.tagit("assignedTags").length&&(a.ul.find(".ui-autocomplete-input").attr("disabled","disabled"),!0===a.is_start&&a.select.focus())},afterTagRemoved:function(d,c){a.changeSelected(a.ul.tagit("assignedTags"),a.removeSelected);1>a.ul.tagit("assignedTags").length&&a.ul.find(".ui-autocomplete-input").removeAttr("disabled")}});
return a}for(var h=f(".smart_tag"),e=h.length-1;0<=e;e--){var l=n(f(h[e]));l.start()}h=f(".single_choice");for(e=h.length-1;0<=e;e--)l=p(f(h[e])),l.start()})})(window.jQuery);